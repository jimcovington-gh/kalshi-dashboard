...
def get_positions_live_comparison(user_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        live_tickers = []
        min_staleness_minutes = float('inf')  # Track FRESHEST update (minimum staleness)
        ...
        for item in live_items:
            ...
            # Calculate staleness - we want the FRESHEST (most recent) update
            if updated_at:
                try:
        ...
        # If no valid timestamps found, set to -1
        if min_staleness_minutes == float('inf'):
            min_staleness_minutes = -1
        
        logger.info(f"COMPARISON: positions-live for {user_name} - {len(live_tickers)} positions, freshest update: {min_staleness_minutes:.1f} min ago")
